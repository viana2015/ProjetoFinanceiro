...
public class CorsFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		response.setHeader("Access-Control-Allow-Origin", jrmoneyProperty.getOriginPermitida());
		response.setHeader("Access-Control-Allow-Credentials", "true");
		
			// Se a origen permitida for igual ao do Browser e for uma requisicao OPTIONS  permiti.
		if ("OPTIONS".equals(request.getMethod()) && jrmoneyProperty.getOriginPermitida().equals(request.getHeader("Origin"))) {
